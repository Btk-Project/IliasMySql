...
EnhancedSqlError::EnhancedSqlError(const SqlError& baseError)
    : mBaseError(baseError) {
}
...
EnhancedSqlError::ErrorContext EnhancedSqlError::getErrorContext() const {
    ...
    if (mConstraintTags.has_value()) {
        ...
        context.violatedConstraints.insert(context.violatedConstraints.end(), 
...
bool EnhancedSqlError::isConstraintViolation() const {
    ...
    return code == SqlError::ConstraintViolation ||
           code == SqlError::PrimaryKeyViolation ||
           code == SqlError::UniqueConstraintViolation ||
           code == SqlError::NotNullViolation ||
           code == SqlError::ForeignKeyViolation ||
           code == SqlError::CheckConstraintViolation;
}
...
std::vector<std::string> EnhancedSqlError::analyzeViolatedConstraints(const SqlTags& tags) const {
    ...
    switch (errorCode) {
        ...
        case SqlError::ConstraintViolation:
            ...
            if (tags.primary_key) violations.push_back("PRIMARY KEY");
            if (tags.not_null) violations.push_back("NOT NULL");
            if (tags.unique) violations.push_back("UNIQUE");
            if (tags.auto_increment) violations.push_back("AUTO_INCREMENT");
            break;
            
        default:
            // For other error types, still report active constraints for context
            if (tags.primary_key) violations.push_back("PRIMARY KEY (context)");
            if (tags.not_null) violations.push_back("NOT NULL (context)");
            if (tags.unique) violations.push_back("UNIQUE (context)");
            break;
    }
...
std::string EnhancedSqlError::generateEnhancedMessage() const {
    ...
    if (mConstraintTags.has_value()) {
        ...
        if (tags.primary_key) activeFlags.push_back("primary_key");
        if (tags.not_null) activeFlags.push_back("not_null");
        if (tags.unique) activeFlags.push_back("unique");
        if (tags.auto_increment) activeFlags.push_back("auto_increment");
        if (tags.index) activeFlags.push_back("index");
        if (tags.unsigned_type) activeFlags.push_back("unsigned_type");
        if (tags.created_at) activeFlags.push_back("created_at");
        if (tags.updated_at) activeFlags.push_back("updated_at");
    ...
    if (!mConstraintContexts.empty()) {
        ...
        for (size_t i = 0; i < mConstraintContexts.size(); ++i) {
            ...
            std::vector<std::string> activeFlags;
            if (tags.primary_key) activeFlags.push_back("PK");
            if (tags.not_null) activeFlags.push_back("NN");
            if (tags.unique) activeFlags.push_back("UQ");
            if (tags.auto_increment) activeFlags.push_back("AI");
...
EnhancedSqlError SqlTagsErrorHandler::enhanceError(const SqlError& error,
                                                  const std::string& tableName,
                                                  const std::string& columnName,
                                                  const SqlTags& tags) {
...
std::optional<SqlError::Code> SqlTagsErrorHandler::mapDatabaseErrorToConstraintType(
    ...
    if (lowerMessage.find("not null") != std::string::npos ||
        lowerMessage.find("null value") != std::string::npos) {
