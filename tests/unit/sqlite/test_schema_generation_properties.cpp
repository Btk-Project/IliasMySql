...
TEST_F(SchemaGenerationPropertyTestFixture, Property1_PrimaryKeyConstraintGeneration) {
    ...
    propertyTester.runPropertyTest(
        [&]() {
            ...
            std::string sqliteRandomColumn =
                Dialect<SqliteTag>::generate_column_definition<int>("test_col", randomTags);
            ...
            std::string postgresRandomColumn =
                Dialect<PostgresTag>::generate_column_definition<int>("test_col", randomTags);
...
// *For any* numeric field configuration with auto_increment = true, the generated SQL schema should contain the
// appropriate auto-increment syntax for the target database
        ...
        [&]() {
            ...
            if (tags.primary_key) {
                ...
                EXPECT_TRUE(propertyTester.containsIgnoreCase(sqliteColumn, "AUTOINCREMENT"))
                    << "SQLite column definition should contain AUTOINCREMENT when primary_key is true: "
                    << sqliteColumn;
            }
