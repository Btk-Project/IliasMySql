...
public:
    ...
    SqlTags generateConstraintTags(bool primary_key = false, bool not_null = false, bool unique = false,
                                   bool auto_increment = false, bool index = false, bool created_at = false) {
        SqlTags tags;
    ...
    bool containsIgnoreCase(std::string_view str, std::string_view substr) {
        ...
        std::string substrLower = std::string(substr);
    ...
    bool isValidPostgreSQLSyntax(std::string_view sql) {
...
// *For any* constraint not supported by the target database, the system should either provide equivalent functionality
...
// *For any* timestamp field configuration, the system should use appropriate timestamp types for the target database
...
TEST_F(PostgreSQLCompatibilityPropertyTestFixture, Property22_DatabaseSpecificTimestampTypes) {
    ...
    propertyTester.runPropertyTest(
        [&]() {
            ...
            std::string postgresTimestamp =
                Dialect<PostgresTag>::generate_column_definition<SqlDate>("timestamp_col", tags);
            ...
            std::string postgresCreatedAt =
                Dialect<PostgresTag>::generate_column_definition<SqlDate>("created_at", tags);
            ...
            if (randomTags.created_at || randomTags.updated_at) {
                std::string randomTimestamp =
                    Dialect<PostgresTag>::generate_column_definition<SqlDate>("ts_col", randomTags);
