...
TEST_F(SqlTagsValidationPropertyTest, Property23_ConflictingConstraintDetection) {
    propertyTester.runPropertyTest(
        [&]() {
            ...
            auto errors  = tags.getValidationErrors<int>();
            bool isValid = tags.isValid<int>();
            ...
            auto randomErrors  = randomTags.getValidationErrors<int>();
            bool randomIsValid = randomTags.isValid<int>();
...
// *For any* configuration with invalid length values (negative numbers), the system should reject the configuration
...
TEST_F(SqlTagsValidationPropertyTest, Property24_InvalidLengthRejection) {
    propertyTester.runPropertyTest(
        [&]() {
            ...
            EXPECT_TRUE(validTags.isValid<std::string>());
...
TEST_F(SqlTagsValidationPropertyTest, ValidationConsistency) {
    propertyTester.runPropertyTest(
        [&]() {
            ...
            bool isValid1 = tags.isValid<int>();
            bool isValid2 = tags.isValid<int>();
            EXPECT_EQ(isValid1, isValid2);

            auto errors1 = tags.getValidationErrors<int>();
            auto errors2 = tags.getValidationErrors<int>();
