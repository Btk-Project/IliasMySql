...
struct Meta<ExtendedUser, void> {
    constexpr static auto value = Object(
        ...
        "email",        makeTags<SqlTags {.index = true}>(&ExtendedUser::email), 
        ...
        "updated_at",   makeTags<SqlTags {.updated_at = true}>(&ExtendedUser::updated_at));
};
template <>
...
public:
    ...
    static auto setup_db() -> IoTask<SqlDatabase> {
        auto open_ret = co_await SqlDatabase::open_in_memory();
        if (!open_ret) {
            ILIAS_ERROR("orm-test", "Failed to open Sqlite: {}", open_ret.error().message());
            throw std::runtime_error("Failed to connect to Sqlite");
        }
    ...
    static auto test_transaction_management() -> IoTask<void> {
        ...
        // 1. 正常事务提交测试
        {
            ...
            auto insert_ret =
                co_await tx.execute("INSERT INTO simple_users (id, name, email, created_at, is_active, balance) "
                                    "VALUES (1, 'TxUser1', 'tx1@test.com', 'no date', true, 100.0)");
            CO_ASSERT_VAL(insert_ret);

            auto insert_ret2 =
                co_await tx.execute("INSERT INTO simple_users (id, name, email, created_at, is_active, balance) "
                                    "VALUES (2, 'TxUser2', 'tx2@test.com', 'no date', true, 200.0)");
        ...
        // 2. 事务回滚测试
        {
            ...
            auto insert_ret =
                co_await tx.execute("INSERT INTO simple_users (id, name, email, created_at, is_active, balance) "
                                    "VALUES (3, 'TxUser3', 'tx3@test.com', 'no date', true, 300.0)");
            CO_ASSERT_VAL(insert_ret);
