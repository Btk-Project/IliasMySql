...
static auto test_invalid_sql_error() -> IoTask<void> {
    ...
    if (!queryRet.has_value()) {
        ...
        EXPECT_TRUE(
            errorCode == SqlError::Code::TableNotFound ||
            errorCode == SqlError::Code::UnknownError
...
static auto test_invalid_parameter_index() -> IoTask<void> {
    ...
    if (!bindRet0.has_value()) {
        EXPECT_EQ(bindRet0.error(), SqlError::Code::InvalidIndex) 
            << "Expected InvalidIndex error for index 0";
    }
    ...
    if (!bindRet2.has_value()) {
        EXPECT_EQ(bindRet2.error(), SqlError::Code::InvalidIndex) 
            << "Expected InvalidIndex error for index 2";
    }
...
static auto test_constraint_violation_error() -> IoTask<void> {
    ...
    auto createRet = co_await db.execute(
    ...
    // Insert a row
    auto insertRet1 = co_await db.execute(
        "INSERT INTO test_error_handling (unique_col) VALUES ('unique_value')"
    );
    CO_ASSERT_VAL(insertRet1);
    
    // Try to insert a duplicate value - should fail with unique constraint violation
    auto insertRet2 = co_await db.execute(
        "INSERT INTO test_error_handling (unique_col) VALUES ('unique_value')"
    );
    ...
    if (!insertRet2.has_value()) {
        ...
        EXPECT_TRUE(
            errorCode == SqlError::Code::UniqueConstraintViolation ||
            errorCode == SqlError::Code::ConstraintViolation ||
            errorCode == SqlError::Code::UnknownError
...
static auto test_invalid_column_index() -> IoTask<void> {
    ...
    EXPECT_TRUE(validValueRet.has_value()) << "Accessing valid column index should succeed";
