...
static auto test_commit_transaction() -> IoTask<void> {
    ...
    // Insert a test row
    auto insertRet = co_await db.execute(
        "INSERT INTO test_transactions (balance) VALUES (999.99)");
    ...
    auto queryRet = co_await db.query(
        "SELECT balance FROM test_transactions WHERE balance = 999.99");
    ...
    auto deleteRet = co_await db.execute(
        "DELETE FROM test_transactions WHERE balance = 999.99");
...
static auto test_rollback_transaction() -> IoTask<void> {
    ...
    // Insert a test row
    auto insertRet = co_await db.execute(
        "INSERT INTO test_transactions (balance) VALUES (888.88)");
    CO_ASSERT_VAL(insertRet);
    ...
    auto queryRet = co_await db.query(
        "SELECT balance FROM test_transactions WHERE balance = 888.88");
    CO_ASSERT_VAL(queryRet);
...
static auto test_transaction_isolation() -> IoTask<void> {
    ...
    auto insertRet = co_await db1.execute(
        "INSERT INTO test_transactions (balance) VALUES (777.77)");
    CO_ASSERT_VAL(insertRet);
    
    // From second connection, the row should NOT be visible (default isolation)
    auto queryRet2 = co_await db2.query(
        "SELECT balance FROM test_transactions WHERE balance = 777.77");
    ...
    auto queryRet3 = co_await db2.query(
        "SELECT balance FROM test_transactions WHERE balance = 777.77");
    ...
    // Clean up
    auto deleteRet = co_await db1.execute(
        "DELETE FROM test_transactions WHERE balance = 777.77");
    CO_ASSERT_VAL(deleteRet);
...
static auto test_sync_rollback() -> IoTask<void> {
    ...
    // Insert a test row
    auto insertRet = co_await db.execute(
        "INSERT INTO test_transactions (balance) VALUES (666.66)");
    CO_ASSERT_VAL(insertRet);
    ...
    // Verify the row was NOT persisted
    auto queryRet = co_await db.query(
        "SELECT balance FROM test_transactions WHERE balance = 666.66");
    CO_ASSERT_VAL(queryRet);
...
static auto test_multiple_transactions() -> IoTask<void> {
    ...
    // First transaction - commit
    {
        ...
        auto insertRet = co_await db.execute(
            "INSERT INTO test_transactions (balance) VALUES (111.11)");
        CO_ASSERT_VAL(insertRet);
    ...
    // Second transaction - rollback
    {
        ...
        auto insertRet = co_await db.execute(
            "INSERT INTO test_transactions (balance) VALUES (222.22)");
        CO_ASSERT_VAL(insertRet);
    ...
    // Third transaction - commit
    {
        ...
        auto insertRet = co_await db.execute(
            "INSERT INTO test_transactions (balance) VALUES (333.33)");
        CO_ASSERT_VAL(insertRet);
    ...
    auto query1 = co_await db.query(
        "SELECT balance FROM test_transactions WHERE balance = 111.11");
    ...
    auto query2 = co_await db.query(
        "SELECT balance FROM test_transactions WHERE balance = 222.22");
    ...
    auto query3 = co_await db.query(
        "SELECT balance FROM test_transactions WHERE balance = 333.33");
    ...
    // Clean up
    auto deleteRet = co_await db.execute(
        "DELETE FROM test_transactions WHERE balance IN (111.11, 333.33)");
    CO_ASSERT_VAL(deleteRet);
