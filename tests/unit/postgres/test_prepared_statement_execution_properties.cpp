...
static auto test_indexed_binding_property() -> IoTask<void> {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        ilias_for_await(auto rc, result.range(retrieved_id, retrieved_name, retrieved_age, retrieved_active)) {
            ...
            EXPECT_EQ(retrieved_name, test_name) 
                << "Name should match bound value";
            EXPECT_EQ(retrieved_age, test_age) 
                << "Age should match bound value";
            EXPECT_EQ(retrieved_active, test_active) 
                << "Active should match bound value";
            EXPECT_GT(retrieved_id, 0) 
...
static auto test_named_binding_property() -> IoTask<void> {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        auto stmtRet = co_await db.prepare(
            "INSERT INTO test_prepared (name, age) VALUES (:name, :age) RETURNING id, name, age"
        );
        CO_ASSERT_VAL(stmtRet);
        auto& stmt = stmtRet.value();
...
static auto test_query_streaming_property() -> IoTask<void> {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        auto stmtRet = co_await db.prepare(
            "SELECT id, value, description FROM test_streaming ORDER BY id LIMIT ?"
        );
        CO_ASSERT_VAL(stmtRet);
        auto& stmt = stmtRet.value();
        ...
        EXPECT_EQ(actual_rows, expected_rows) 
            << "Should return exactly " << expected_rows << " rows";
    }
...
static auto test_execute_affected_rows_property() -> IoTask<void> {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        auto cleanupStmtRet = co_await db.prepare(
            "DELETE FROM test_prepared WHERE age = ? OR age = ?"
        );
        ...
        for (int j = 0; j < insert_count; ++j) {
            ...
            auto insertStmtRet = co_await db.prepare(
                "INSERT INTO test_prepared (name, age) VALUES (?, ?)"
            );
        ...
        auto stmtRet = co_await db.prepare(
            "UPDATE test_prepared SET age = age + 1 WHERE age = ?"
        );
        CO_ASSERT_VAL(stmtRet);
        auto& stmt = stmtRet.value();
        ...
        auto verifyStmtRet = co_await db.prepare(
            "SELECT COUNT(*) FROM test_prepared WHERE age = ?"
        );
...
static auto test_multiple_param_types_property() -> IoTask<void> {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        auto stmtRet = co_await db.prepare(
            "SELECT ?::integer as int_val, ?::double precision as float_val, ?::boolean as bool_val, ?::text as text_val"
        );
        CO_ASSERT_VAL(stmtRet);
        auto& stmt = stmtRet.value();
        ...
        ilias_for_await(auto rc, result.range(retrieved_int, retrieved_float, retrieved_bool, retrieved_string)) {
            ...
            EXPECT_EQ(retrieved_int, test_int) 
                << "Integer should match";
