...
static auto convert_binary_data(std::span<SourceType> source_data, TargetType &value) -> IoResult<void> {
    ...
    if constexpr (is_view_v<CleanTargetType>) {
        ...
        if (source_data.size_bytes() % sizeof(ViewElementType) != 0) {
            ILIAS_WARN("ilias-sql", "Type not matched: {} -> {}", source_data, typeid(TargetType).name());
    ...
    // --- 分支3: 目标是固定大小缓冲区 (C-array, std::array) (拷贝) ---
        ...
        if (bytes_to_copy < source_data.size_bytes()) {
            ILIAS_WARN("ilias-sql", "Data truncated: {} -> {}", source_data, typeid(TargetType).name());
    ...
    static auto convert(SqlValueView &ret, U &value) -> IoResult<void> {
        ...
        switch ((SqlValueType)ret.index()) {
            case SqlValueType::kChar: {
                if constexpr (std::is_convertible_v<SqlValueTraits<SqlValueType::kChar>::viewType, T>) {
            ...
            case SqlValueType::kInt: {
                if constexpr (std::is_convertible_v<SqlValueTraits<SqlValueType::kInt>::viewType, T>) {
            ...
            case SqlValueType::kBigInt: {
                if constexpr (std::is_convertible_v<SqlValueTraits<SqlValueType::kBigInt>::viewType, T>) {
            ...
            case SqlValueType::kFloat: {
                if constexpr (std::is_convertible_v<SqlValueTraits<SqlValueType::kFloat>::viewType, T>) {
            ...
            case SqlValueType::kDouble: {
                if constexpr (std::is_convertible_v<SqlValueTraits<SqlValueType::kDouble>::viewType, T>) {
            ...
            case SqlValueType::kText: {
                auto text_data = get<SqlValueType::kText>(ret);
                return detail::convert_binary_data(std::span {text_data.data(), text_data.size()}, value);
            }
            case SqlValueType::kBlob: {
                return detail::convert_binary_data(get<SqlValueType::kBlob>(ret), value);
            }
        ...
        ILIAS_WARN("ilias-sql", "Type not matched: {}({}) -> {}", ret, getSqltypeName(ret), typeid(U).name());
        // 如果类型不匹配或未处理
        return Unexpected(SqlError::TypeNotMatched);
    }
};
