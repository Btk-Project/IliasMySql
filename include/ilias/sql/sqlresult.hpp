...
public:
    ...
    template <typename... Args>
        requires(!nekoproto::NamedReflectable<std::decay_t<Args>> && ...)
    auto range(Args &...args) -> Generator<IoResult<void>>;
    template <typename... Args>
        requires(sizeof...(Args) > 0 && (nekoproto::NamedReflectable<std::decay_t<Args>> && ...))
...
auto SqlResult<T>::rangeResult() -> Generator<IoResult<T>> {
    T value;
    if constexpr (nekoproto::TupleLike<T>) {
