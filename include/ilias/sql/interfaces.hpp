...
public:
    ...
    virtual auto bind(size_t index, SqlValueView value) -> IoResult<void> = 0;
    virtual auto bind(std::string_view name, SqlValueView value) -> IoResult<void> = 0;
...
public:
    ...
    // 直接执行 SQL (不带参)
    virtual auto exec(std::string_view sql) -> IoTask<void> = 0;

    // 事务控制
    virtual auto beginTransaction() -> IoTask<void> = 0;
    virtual auto commit() -> IoTask<void> = 0;
    virtual auto rollback() -> IoTask<void> = 0;
