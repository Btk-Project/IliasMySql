...
public:
     ...
     */
    EnhancedSqlError(const SqlError &baseError, std::string_view tableName, std::string_view columnName,
                     const SqlTags &constraintTags);

    /**
     * @brief Construct enhanced error with multiple constraint contexts
     */
    EnhancedSqlError(const SqlError &baseError, std::string_view tableName,
     ...
     */
    const std::vector<std::pair<std::string, SqlTags>> &getConstraintContexts() const { return mConstraintContexts; }
...
public:
     ...
     */
    static EnhancedSqlError enhanceError(const SqlError &error, std::string_view tableName, std::string_view columnName,
                                         const SqlTags &tags);

    /**
     * @brief Enhance a SqlError with multiple constraint contexts
     */
    static EnhancedSqlError enhanceError(const SqlError &error, std::string_view tableName,
     ...
     */
    static std::optional<SqlError::Code> mapDatabaseErrorToConstraintType(std::string_view databaseErrorMessage);
